...
<html>
	...
	function check(){
		...
		// 취미 2개이상 선택
		var n=0;
		if(na.hobby[0].checked) n++;
		if(na.hobby[1].checked) n++;
		if(na.hobby[2].checked) n++;
		if(na.hobby[3].checked) n++;
	...
	주소:<input type=text size=50 name=address><br><br>	
	자기소개:<textarea rows=5 cols=35	name=intro></textarea><br><br>
	직업:<select name=job>
			<option>선택
			<option>회사원
			<option>공무원
