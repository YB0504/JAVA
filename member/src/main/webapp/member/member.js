...
/*	// ID중복검사
	$("#idcheck").click(function() {
		if ($("#id").val() == "") {
			...
			window.open(ref, "mywin", "width=450,height=650");
		}
	});*/
	...
	$("#idcheck").click(function(){
		if($("#id").val() == ""){
			...
			$.ajax({
				type : "post",			// 값을 전달할 방식 : get, post, put(수정), delete(삭제)
				url : "idcheck1.jsp",	// 요청할 파일 URL명
				data : {"id" : id},		// json형태로 값을 가져간다.
				success : function(data){
					alert(data);
					if(data == 1){		// 중복 ID
						$("#myid").html("<font color=red>중복된 ID입니다.</font>");
						$("#id").val("").focus();
					}else{				// 사용 가능한 ID
