...
public class BoardDBBean {
	...
	// 총 데이터 갯수 구하기
	// 검색창에서 검색된 데이터의 개수와 총데이터 개수의 sql문을 따로 작성한다.
	public int getCount(String sel, String find) {
		...
		try {
			...
			if(sel != null && find != null) { // 검색한 데이터 개수
				sql = "select count(*) from board ";
				// 검색어의 항목과 맞는 컬럼명이 sel변수에 전달되어 들어간다.
				// 검색될 내용을 찾기 위해 like연산자와 %와일드 카드를 사용한다.
	...
	// 검색한 데이터의 목록을 추출할때는 if ~ else문으로 처리
	public List<BoardDataBean> getList(int start, int end, String sel, String find) {
		...
		try {
			...
			if(sel != null && find != null) {
				...
				sql += " where " + sel +" like '%" + find + "%' ) ";	// like 연산자 사용
